...
public class oldpWeaponMgr : MonoBehaviour {
	...
	void FireMult(){
		if (!zapperOut){
            ...
            zapper.GetComponent<oldMultizapper>().WeaponManager = this;
            lastBullet = Time.timeSinceLevelLoad;
            zapperOut = true;
		}
	}
	...
	void FireFac(){
		if (Time.timeSinceLevelLoad - lastBullet > 1/factorBeamFireSpeed && GetComponent<oldpMoveMgr>().CurrentJuice
		     >= factorBeamJuiceCost){
			GetComponent<oldpMoveMgr>().CurrentJuice -= factorBeamJuiceCost;
