...
public class oldpShieldMgr : MonoBehaviour {
	...
	void FixedUpdate(){
		ManageShield();
		if (GetComponent<oldpMoveMgr>().CurrentJuice > delayThreshold)
			shieldDelay = false;
		else if(!shieldOn)
			shieldDelay = true;
	}
	...
	void CheckInputs(){
		if (Input.GetButton ("Shield") && GetComponent<oldpMoveMgr>().CurrentJuice > 0  && !shieldDelay){
			shieldOn = true;
